...
import { useGetParentsQuery, useUpdateParentsMutation } from '../Slices/userApiSlice'; // Adjust the import path accordingly
import { useDispatch, useSelector } from 'react-redux';
import { setCredentials } from '../Slices/authSlice';
import { useNavigate } from 'react-router-dom';
import { toast } from 'react-toastify';
import regpic from '../assets/Parents.jpg';
...
const ParentProfile = () => {
  ...
  const [studentId, setStudentId] = useState(''); // Added field for Student ID
  const [course, setCourse] = useState('');
  const [gender, setGender] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);

  const { data: parentData, isLoading: isLoadingParent, error } = useGetParentsQuery(userInfo?.id);
  const [updateParent, { isLoading: isUpdatingParent }] = useUpdateParentsMutation();
  ...
  useEffect(() => {
    if (parentData) {
      ...
      setStudentId(parentData.studentId); // Ensure studentId is set here
      setCourse(parentData.course);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await updateParent({
        ...
        email,
        address,
        studentId, // Include Student ID in the submission data
        course,
        gender,
        password,
      }).unwrap();
  ...
  if (error) {
    return <div>Error loading parent data</div>;
  }
